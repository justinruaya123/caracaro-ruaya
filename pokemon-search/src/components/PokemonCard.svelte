...
{:then data}
    <div class="card max-w-[512px] min-w-[512px] m-4 p-4 min-h-80 card-hover rounded-lg variant-filled-success">
        <div class="header flex justify-left items-center">
            <Avatar src={data.sprites.front_default} alt={data.name} width="w-32" />
            <span class="m-10 text-3xl">{data.name.charAt(0).toUpperCase() + data.name.slice(1).replace('-', ' ')} ({data.id})</span>
        </div>
        <section>
